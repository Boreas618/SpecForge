...
- **Regenerated data is a verified artifact.** `_temp/regen` is promoted into the generic
  `specforge/data/regen` runtime. It captures decoded text messages and structured reasoning/tool
  trajectories only—never hidden states, logits, embeddings, or tensor features. First-party
  model-derived rows reach consumers through a finalized, content-addressed `DatasetArtifact`.
...
| Data regeneration | [data-regeneration.md](./data-regeneration.md) | General text/reasoning trajectory workflows, exact resume/rebuild, validation, artifacts, provenance |
...
| R0 — Contract freeze and publication audit | regeneration | S | **implemented** (this change set) |
| R1 — Generic kernel and artifact lifecycle | regeneration | L | **implemented** (this change set) |
| R2 — Adapter migration and vertical-slice parity | regeneration | L | **implemented** (Inkling exact-raw slice = external codec plugin) |
| R3 — Composition, CLI, and required dataset boundary | regeneration | L | **implemented** (this change set) |
| R4 — Distributed scale and operational hardening | regeneration | L | **implemented** (cloud blob adapter = deployment work) |
| R5 — Operation and trajectory breadth | regeneration | L | **implemented** (live sandboxes = deployment work) |
...
- Regeneration is independent of the feature-capture runtime. **R3** coordinates only with the
  typed config/CLI and text-dataset preparation boundary; no regeneration phase depends on
  hidden-state capture or feature transport.
