...
Sibling tracks: [domain / architecture](./domain-refactor.md) ·
[online disaggregation](./online-disaggregation.md) ·
[eval & breadth](./eval-and-breadth.md) · root [architecture plan](../../plan.md).
...
- `scripts/regenerate_train_data.py` becomes a one-release compatibility wrapper that
constructs a replay recipe and calls the same pipeline.
- `scripts/validate_regenerated_data.py` becomes a wrapper around the baseline validation
profile.
...
artifact has equivalent or stronger accounting.
...
- **Implementation:**
  ...
  - Convert old scripts to compatibility wrappers with deprecation messages and convert examples
  to recipes. Remove `_temp/regen` only after parity and documentation gates pass.
