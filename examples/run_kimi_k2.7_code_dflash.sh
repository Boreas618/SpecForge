#!/bin/bash
# =============================================================================
# Kimi-K2.7-Code DFlash — single-script, full-cycle, 2-node trainer
# =============================================================================
# Trains the DFlash drafter for moonshotai/Kimi-K2.7-Code online, with the
# SGLang target backend, across 2 nodes (16x H200). One script covers the whole
# cycle: environment setup, data prep, training, eval, checkpointing, resume —
# launched over ssh OR slurm. Hand this single file (+ the repo) to a collaborator.
#
# Recipe follows z-lab/Kimi-K2.5-DFlash (block diffusion drafter, block_size=8,
# 6 full-attention draft layers over target hidden states captured at layers
# [1,12,24,35,47,58]). Kimi-K2.7-Code shares K2.5's text dims (61 layers, hidden
# 7168, vocab 163840, YaRN rope), so configs/kimi-k2.7-code-dflash.json transfers
# verbatim.
#
# TOPOLOGY (DeepSeek/Kimi-style large-MoE serving — set per request):
#   tp_size = ep_size = dp_size = WORLD (=16): ONE target instance across both
#   nodes. DP-attention -> each rank runs its own sequences' attention (no
#   attention all-reduce); experts are EP-16 with the `deepep` all-to-all over
#   InfiniBand. Per-GPU expert weights drop to ~37 GB (vs ~74 GB at tp=8).
#   The training dataloader shards 16-way over the world (train_dflash detects
#   --sglang-enable-dp-attention), so the 16 ranks consume DISTINCT samples and
#   FSDP grad-averaging yields a true 16-way data-parallel gradient.
#   => Effective global batch = WORLD * BATCH_SIZE = 16 * 4 = 64 sequences/step.
#      This is ~32x the old tp=8/dp=2/batch=1 setup; SCALE THE LR ACCORDINGLY
#      (LEARNING_RATE env; default 6e-4 is the per-(small-batch) starting point).
#
# -----------------------------------------------------------------------------
# USAGE — you ssh from your LOCAL machine into EACH node (no node->node ssh
#         needed); the repo lives on a shared FS both nodes mount.
#   On EACH node, once:        bash examples/run_kimi_k2.7_code_dflash.sh setup       # deps (node-local)
#   On the rank-0 node, once:  bash examples/run_kimi_k2.7_code_dflash.sh prepare   # model+data+cache (shared FS)
#   Launch — run on EACH node in its own ssh session, inside tmux so it survives
#   disconnect. The rank-0 host is the rendezvous master:
...
#   (use `train` instead of `watchdog` for a one-shot foreground run / smoke test.)
#
# ALTERNATIVE LAUNCHERS
#   slurm (from a login node):  bash run_kimi_k2.7_code_dflash.sh slurm
#   ssh fan-out (ONLY if your launch host has passwordless ssh to BOTH nodes; set
#     REMOTE_ROOT if the repo path on the nodes differs from where you run this):
#       bash run_kimi_k2.7_code_dflash.sh ssh <rank0-host> <rank1-host>
#
# KEY ENV (all overridable):
#   NNODES=2 NUM_GPUS=8 MASTER_ADDR=<rank0 host> MASTER_PORT=29500
#   BATCH_SIZE=4 LEARNING_RATE=6e-4 MEM_FRACTION=0.6 REPORT_TO=tensorboard
#   HF_TOKEN=<...>   (for `prepare` — gated model; never hard-coded here)
#   PARTITION/TIME   (for `slurm`)
# =============================================================================
# This script uses bash-only features (arrays, heredocs). Re-exec under bash if
# someone runs it via `sh script.sh` (where /bin/sh may be dash) — otherwise it
# would fail to parse. Must precede `set -o pipefail`, which dash cannot parse.
...
NNODES=${NNODES:-2}
NUM_GPUS=${NUM_GPUS:-8}
WORLD=$((NNODES * NUM_GPUS))               # tp = ep = dp = WORLD (single target instance)
...
# sglang static pool (weights + KV). Training only does prefill capture, so the
# KV pool can be small; 0.8 over-reserved it and OOM'd the INT4 MoE marlin
# intermediate (~18 GB). 0.6 leaves headroom for it + the draft/optimizer.
...
# For the `ssh` fan-out only: repo path ON THE NODES (defaults to this checkout's
# path, which is correct when the repo is on a shared FS both nodes mount).
...
# ===========================================================================
cmd_setup() {
  # Per-node, once. The lmsysorg/sglang image ships torch/sglang/transformers/
  # flashinfer/flash-attn/deepep; pyproject pins OLDER versions, so a plain
  # `pip install -e .` would downgrade and break CUDA. Install --no-deps + the
  # missing light deps only.
  cd "$ROOT_DIR"
  log "setup: pip install --no-deps -e . + light deps"
...
PY
  ...
  log "  rank1:  NODE_RANK=1 MASTER_ADDR=<rank0-host> bash $0 watchdog   (or: $0 slurm)"
}

# ===========================================================================
cmd_train() {
  # Run on ONE node. NODE_RANK / NNODES / MASTER_ADDR come from the env, or are
  # derived from SLURM_* when launched under srun.
  if [ -n "${SLURM_NODEID:-}" ]; then
    NODE_RANK=${NODE_RANK:-$SLURM_NODEID}
    NNODES=${SLURM_NNODES:-$NNODES}
    WORLD=$((NNODES * NUM_GPUS))
    [ -n "$MASTER_ADDR" ] || MASTER_ADDR=$(scontrol show hostnames "$SLURM_JOB_NODELIST" | head -n1)
  fi
...
# ===========================================================================
cmd_ssh() {
  # OPTIONAL convenience: launch from a host with passwordless ssh to BOTH nodes.
  # (Not needed if you ssh into each node yourself and run `watchdog` there.)
  # Remote paths use $REMOTE_ROOT (the repo path on the nodes), NOT the local path,
  # so this works even when launched from a laptop without the repo. First arg =
  # rank-0 host (rendezvous master).
  local nodes=("$@")
  [ "${#nodes[@]}" -ge 1 ] || { echo "usage: $0 ssh <rank0-host> [rank1-host ...]"; exit 1; }
  local master="${nodes[0]}" n="${#nodes[@]}"
  local remote_script="$REMOTE_ROOT/examples/$(basename "$SCRIPT_PATH")"
  local remote_out="$REMOTE_ROOT/outputs/kimi-k2.7-code-dflash-nemotron"
  for i in "${!nodes[@]}"; do
    local host="${nodes[$i]}"
    log "ssh: launching node_rank=$i on $host (master=$master, REMOTE_ROOT=$REMOTE_ROOT)"
    ssh -o BatchMode=yes "$host" "mkdir -p '$remote_out' && cd '$REMOTE_ROOT' && \
      setsid env NODE_RANK=$i NNODES=$n MASTER_ADDR='$master' MASTER_PORT='$MASTER_PORT' \
        NUM_GPUS='$NUM_GPUS' BATCH_SIZE='$BATCH_SIZE' LEARNING_RATE='$LEARNING_RATE' \
        MEM_FRACTION='$MEM_FRACTION' REPORT_TO='$REPORT_TO' \
        bash '$remote_script' watchdog >> '$remote_out/train.rank$i.log' 2>&1 < /dev/null &" \
      && log "  started (log on $host: $remote_out/train.rank$i.log)" \
      || log "  FAILED to ssh $host"
  done
  log "ssh: launched. Tail with: ssh $master tail -f $remote_out/train.rank0.log"
}

# ===========================================================================
cmd_slurm() {
  # Submit a 2-node job; each node task re-enters this script as `train`,
  # deriving NODE_RANK/MASTER_ADDR from SLURM_*.
  mkdir -p "$OUTPUT_DIR"
  local extra=()
  [ -n "${PARTITION:-}" ] && extra+=(--partition="$PARTITION")
  [ -n "${TIME:-}" ] && extra+=(--time="$TIME")
  set -x
  sbatch --job-name=kimi-dflash --nodes="$NNODES" --ntasks-per-node=1 \
    --gpus-per-node="$NUM_GPUS" "${extra[@]}" \
    --output="$OUTPUT_DIR/slurm-%j.out" \
    --wrap "srun --ntasks-per-node=1 bash '$SCRIPT_PATH' train"
}
