...
DATASETS = {
    "gsm8k": dict(load_args=("openai/gsm8k", "main"), split="test",
    ...
    "mt-bench": dict(load_args=("HuggingFaceH4/mt_bench_prompts",), split="train",
                     fmt=lambda r: list(r["prompt"])),
    "humaneval": dict(load_args=("openai/openai_humaneval",), split="test",
                      fmt=lambda r: ["Write a solution to the following problem and make "
                                     "sure that it passes the tests:\n```python\n"
                                     + r["prompt"] + "\n```"]),
    "mbpp": dict(load_args=("google-research-datasets/mbpp", "sanitized"), split="test",
                 fmt=lambda r: [r["prompt"]]),
    "lbpp": dict(load_args=("CohereLabs/lbpp",), split="test",
                 fmt=lambda r: [r["instruction"]],
                 parquet_files=("python/test.parquet",)),
    "swe-bench": dict(load_args=("princeton-nlp/SWE-bench_Lite",), split="test",
                      fmt=lambda r: ["Problem Statement:\n" + r["problem_statement"]
                                     + "\nPlease fix the issue described above."]),
    "livecodebench": dict(load_args=("livecodebench/code_generation_lite",), split="test",
                          fmt=fmt_livecodebench,
                          jsonl_files=("test.jsonl", "test2.jsonl", "test3.jsonl",
...
def _load_rows(name):
    ...
    if cfg.get("loader") == "arena_hard":
        ...
        src = hf_hub_download(repo_id="lmarena-ai/arena-hard-auto",
    ...
    if cfg.get("jsonl_files"):
        ...
        for fn in cfg["jsonl_files"]:
            src = hf_hub_download(repo_id=cfg["load_args"][0], filename=fn,
                                  repo_type="dataset")
            with open(src, encoding="utf-8") as f:
...
def run_workload(name, tokenizer, args):
    ...
    if args.warmup:
        with ThreadPoolExecutor(max_workers=args.warmup) as pool:
            list(pool.map(lambda p: send_one(args.base_url, p, args), prompts[:args.warmup]))
...
def main():
    ...
    ap.add_argument("--output-json", default="/scratch/bench_cache/accept_results.json")
    args = ap.parse_args()
    ...
    for r in results:
        print("%-16s %.4f  (n=%d)" % (r["workload"], r["mean_accept_length"],
                                      r["with_accept_meta"]))
