...
def test_canonical_json_is_deterministic_and_order_independent():
    left = canonical_json({"b": 1, "a": [3, 2, {"y": 1, "x": 2}]})
    right = canonical_json({"a": [3, 2, {"x": 2, "y": 1}]})
    # Key insertion order never changes the canonical form.
    assert left == canonical_json({"a": [3, 2, {"y": 1, "x": 2}], "b": 1})
    assert left != right
    assert canonical_json({"a": [3, 2, {"x": 2, "y": 1}]}) == right
